    ...
    try:
        for step in range(1, max_steps + 1):
            ...
            if kind == "done":
                ...
                break
            if kind == "malformed":
                # すでに有効な編集が1つ以上適用済みで、追加応答が実体を持たない
                # （閉じフェンス ``` だけ・空応答 等）の場合は、DONE を出さない小型
                # モデルが「実質完了」している状況とみなして保存する。ここでループを
                # 続けても手を浪費し、最終的にステップ上限で編集ごと破棄されてしまう
                # ため、得られた成果を残す方が良い（続きはチャットの次の指示で足せる）。
                if applied > 0:
                    completed = True
                    _log(
                        f"[結果] MALFORMED だが適用済み編集あり → 完了として保存 "
                        f"(applied={applied})"
                    )
                    break
                # まだ何も編集できていない → コード or DONE を促して再試行
