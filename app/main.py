...
class LLMClient:
    ...
    def agent_step(
        ...
        prompt = (
            ...
            "helpers が使えます。\n"
            "応答ルール:\n"
            "  ・実行するコードは ```python で始め ``` で必ず閉じる1ブロックにする"
            "（閉じ忘れ厳禁。print()で結果確認可）。\n"
            "  ・そのコードで指示が完了するなら、閉じる ``` の後ろの行に DONE だけ書く"
            "（1回で完了でき高速）。まだ続くときだけ DONE を付けない。\n"
            ...
            "pat = re.compile(r'[ァ-ヶ゠-ヿ]')  # カタカナ\n"
            "for row in ws.iter_rows():\n"
            "    for c in row:\n"
            "        if c.value is not None and pat.search(str(c.value)):\n"
            "            c.fill = fill\n"
            "```\n"
            "DONE\n"
            f"指示: {instruction}\n"
